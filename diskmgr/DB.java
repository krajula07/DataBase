...
public class DB implements GlobalConst {
  ...
  public  void read_page(PageId pageno, Page apage)
    ...
    try{
      fp.read(buffer);
     // System.out.println("Read from the pagenumber:"+pageno.pid);
  ...
  public void write_page(PageId pageno, Page apage)
    ...
    try{
      ...
      //System.out.println("Written data into page number:"+pageno.pid);
  ...
  public void add_file_entry(String fname, PageId start_page_num)
    ...
    if(fname.length() >= MAX_NAME) //changed from MAX_NAME to 50
   ...
   // System.out.println("name errorrrrrrrrrrrrrrrrrrrrrrrrrr"+fname);
	...
	pinPage(hpid, apage, false/*no diskIO*/); // changed from true
	Arrays.fill(apage.data, (byte)0);
...
class DBHeaderPage implements PageUsedBytes, GlobalConst { 
  ...
  public DBHeaderPage(Page page, int pageusedbytes)
    throws IOException
    {
      ...
      System.out.println("number of entries background:"+num_entries+":"+"max space"+MAX_SPACE+"Size of file entry"+SIZE_OF_FILE_ENTRY);
  ...
  public String getFileEntry(PageId pageNo, int entryNo)
   ...
   // System.out.println("pid:"+pageNo.pid+"position:"+position);
    return (Convert.getStrValue (position+4, data, MAX_NAME + 2)); //changed from MAX_NAME to 50
  }
