...
public class Heapfile implements Filetype,  GlobalConst {
  ...
  public     int 	 _indexType;
  private static int tempfilecount = 0;
  private int rowCnt; //number of distinct rows
  private int columnCnt; // number of distinct columns
  ...
  */
  private boolean  _findDataPage( MID mid,
	...
	{// Start While01
	  ...
	  for( currentDataPageRid = currentDirPage.firstRecord();
	    ...
	    {
	      try{
		amap = currentDirPage.getRecord(currentDataPageRid);
	      }
  ...
  public  Heapfile(String name) 
	...
	{
	//  System.out.println(_fileName + " file doesn't exist. First create it.");
	  _firstDirPageId = newPage(apage, 1);
	  // check error
	  if(_firstDirPageId == null)
	    throw new HFException(null, "can't new page");
	//  System.out.println("testing why is it failig?"+_fileName+":"+_firstDirPageId);
  ...
  public Heapfile(String name, int type) throws HFException, HFBufMgrException, HFDiskMgrException, IOException
  {
	   this(name);
	  _indexType=type;
   ...
   */
  // HashMap<String, List<MID>> hashmap = new HashMap<>();
  public MID insertRecord(byte[] recPtr) 
    throws Exception
    {
	  //Uncomment the below for 3 condition check and uncomment the last two lines of this method
  ...
  public Scan openScan() 
	   ...
	   IOException
    {
      ...
      System.out.println("in open scan in heap file");
      return newscan;
    }
  ...
  public void deleteFile()  
	...
	{      
	  ...
	  // the current directory page.
	  //System.out.println("unpin pages: "+currentDirPageId);
      nextDirPageId = currentDirPage.getNextPage();
     // System.out.println("unpin pages: "+currentDirPageId);    
