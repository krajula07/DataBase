  ...
  implements GlobalConst, Catalogglobal
{
  ...
  public int getRelInfo(String relation, int indexCnt, IndexDesc [] indexes)
      ...
      while(true) 
	{
	  try {
	    ...
	    read_map(map, indexes[count]);
	  }
	  catch (Exception e4) {
	    throw new IndexCatalogException(e4," read_map() failed");
	  }
  ...
  public void addInfo(IndexDesc record)
	...
	throw new IndexCatalogException(e4, "make_map failed");
      }
  ...
  public void addIndex(String relation, String attrName,
	...
	throw new IndexCatalogException(e,"openScan() failed");
      ...
      try {
	ExtendedSystemDefs.MINIBASE_ATTRCAT.getMapStructure(relation, attrCnt, typeArray, sizeArray);
      }
      catch (Exception e) {
	throw new IndexCatalogException(e,"getMapStructure");
      }
  ...
  void make_map(Map map, IndexDesc record)
    throws IOException,
	   IndexCatalogException
    {
      try {
	...
	if (record.order.mapOrder == MapOrder.Ascending)
	  ...
	  if (record.order.mapOrder == MapOrder.Descending)
	    ...
	    if (record.order.mapOrder == MapOrder.Random)
	      ...
	      System.out.println("Invalid order in IndexCatalog::make_map");
	...
	throw new IndexCatalogException(e,"make_map failed");
      }
      
      return;
    };
  
  void read_map(Map map, IndexDesc record)
    throws IOException,
	   IndexCatalogException
    {
      try {
	...
	if (temp == 0)
	  ...
	  if (temp == 1)
	    ...
	    if (temp == 2)
	      ...
	      System.out.println("111Error in IndexCatalog::read_map");
	...
	if (temp == 0)
	  ...
	  if (temp == 1)
	    ...
	    if (temp == 2)
	      ...
	      System.out.println("222Error in IndexCatalog::read_map");
	...
	throw new IndexCatalogException(e,"read_map failed");
      }
