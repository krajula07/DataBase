 ...
 * to clean up.
 */
public class BDSort extends BDIterator implements GlobalConst
{
  ...
  private int          n_runs;
  private Map        op_buf;
  private BDOBuf         o_buf;
  private BDSpoofIbuf[]  i_buf;
	   ...
	   Exception
  {
    ...
    int i;
    BDpnode cur_node;  // need pq_defs.java
    
    i_buf = new BDSpoofIbuf[n_R_runs];   // need io_bufs.java
    for (int j=0; j<n_R_runs; j++) i_buf[j] = new BDSpoofIbuf();
	...
	throw new BDSortException(e, "Sort.java: Map.setHdr() failed");
      }
	...
	*/
	...
	try {
	  ...
	  throw new BDSortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
	}
	catch (BDUtilsException e) {
	  throw new BDSortException(e, "Sort.java: MapUtilsException caught from Q.enq()");
	}
  ...
  { 
    ...
    Map map = new Map();
    try {
    	map.setRowLabel("ZZZZZZZZ");
    	map.setColumnLabel("ZZZZZZZZ");
    	map.setTimeStamp(Integer.MAX_VALUE);
    	map.setValue("ZZZZZZZZ");
	   ...
	   Exception
  {
    Map map; 
    BDpnode cur_node;
    BDpnodeSplayPQ Q1 = new BDpnodeSplayPQ(order);
    BDpnodeSplayPQ Q2 = new BDpnodeSplayPQ(order);
    BDpnodeSplayPQ pcurr_Q = Q1;
    BDpnodeSplayPQ pother_Q = Q2; 
    Map lastElem = new Map(map_size);  // need map.java
    try {
      lastElem.setHdr(n_cols, _in, str_lens);
    }
    catch (Exception e) {
      throw new BDSortException(e, "Sort.java: setHdr() failed");
    }
    ...
    lastElem=createmaxDummyLastElement();
   // int count=0;
	...
	e.printStackTrace(); 
	throw new BDSortException(e, "Sort.java: get_next() failed");
      } 
...
//System.out.println("checking the values in map:"+cur_node.map.getColumnLabel()+":"+cur_node.map.getRowLabel()+":"+cur_node.map.getTimeStamp()+":"+cur_node.map.getValue());
  ...
  //    count++;
   ...
   // System.out.println("elements in count:"+count);
 //  System.out.println("before queue.................."+p_elems_curr_Q);
    ...
    if (cur_node == null) break; 
     ...
     // System.out.println("order:"+order+"last element:"+lastElem.getRowLabel()+":"+lastElem.getColumnLabel()+":"+lastElem.getTimeStamp()+":"+lastElem.getValue()+"current node"+cur_node.run_num+";"+cur_node.map.getRowLabel()+":"+cur_node.map.getColumnLabel()+":"+cur_node.map.getTimeStamp()+":"+cur_node.map.getValue());
      ...
      comp_res = BDUtils.CompareMapWithMap(order, cur_node.map, lastElem);  // need
  ...
  //   System.out.println(comp_res);
   ...
   // if ((comp_res < 0 && order.mapOrder == MapOrder.Ascending) || (comp_res > 0 && order.mapOrder == MapOrder.Descending)) {
    if((comp_res > 0 && order.mapOrder == MapOrder.Descending)){
	...
	//BDUtils.SetValue(lastElem, cur_node.map, _sort_fld, sortFldType);
	BDUtils.SetValue(lastElem,cur_node.map);
...
//	System.out.println("run number and tempheapfiles"+run_num+":"+n_tempfiles);
	...
	try {
	  ...
	  throw new BDSortException(e, "Sort.java: create Heapfile failed");
	}
	...
	p_elems_other_Q = tempelems;
      ...
      // now check whether the current queue is empty
    	...
    	//  System.out.println("in second else part");
	while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
	  try {
		  MID mid=new MID();
	    map = _am.getNext(mid);  // according to Iterator.java
	  } catch (Exception e) {
	    throw new BDSortException(e, "get_next() failed");
	  } 
  ...
  private Map delete_min() 
    throws IOException, 
	   ...
	   Exception
  {
    BDpnode cur_node;                // needs pq_defs.java  
    Map new_map, old_map;  
	...
	*/
	...
	throw new BDSortException("********** Wait a minute, I thought input is not empty ***************");
      }
  ...
  public BDSort(AttrType[] in,         
	      ...
	      int        sort_fld_len,  
	      int        n_pages      
	      ) throws IOException, BDSortException
  {
    ...
    try {
      ...
      throw new BDSortException(e, "Sort.java: t.setHdr() failed");
    }
    map_size = 54;
	...
	throw new BDSortException(e, "Sort.java: BUFmgr error");
      }
    }
    else {
...
//      System.out.println(temp_files[0].getRecCnt()+"record count in sort");
      ...
      throw new BDSortException(e, "Sort.java: Heapfile error");
    }
    ...
    try {
      ...
      throw new BDSortException(e, "Sort.java: op_buf.setHdr() failed");
    }
...
try {
   ...
   * @exception SortException something went wrong in the lower layer. 
   */
  public void close() throws BDSortException, IOException
  {
    // clean up
    if (!closeFlag) {
	...
	throw new BDSortException(e, "Sort.java: error in closing iterator.");
      }
