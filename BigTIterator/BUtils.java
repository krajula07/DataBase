...
public class BUtils
{
	...
	private static int compareTimeStampDesc(Map t1, Map t2)
	  {	
		 /* try
		  ...
		  try {
			  int t1_i,t2_i;
			    t1_i = t1.getTimeStamp();
			    t2_i = t2.getTimeStamp();
	...
	private static int compareValue(Map t1, Map t2)
	  {	
		  ...
		  { 
			  ...
			  t1_f = t1.getValue();			// Comparing column
			  t2_f = t2.getValue();
			 /* int i=Integer.parseInt(t1_f);
			  int j=Integer.parseInt(t2_f);
			  if (t1_i <  t2_i) return -1;
			  if (t1_i >  t2_i) return  1;*/
			 // return 0;
			  int t=t1_f.compareTo(t2_f);
	...
	private static int compareValueTrail(Map t1, Map t2)
	  {	
		  try
		  { 
			  String t1_f, t2_f;
				...
				//  System.out.println("number of order type"+orderType.mapOrder);
				  ...
				  case 2:              // R, C, T
					  ...
					  case 7:                		// T
						  ...
						  return retVal;
					  case 8:                		// T
						  retVal = compareValueTrail(t1, t2);
   ...
   *@exception BUtilsException exception from this class
  ...
  public static int CompareMapWithMap(AttrType fldType,
	...
	case AttrType.attrInteger:                // Compare two integers.
	  try {
	    ...
	    throw new BUtilsException(e, "FieldNumberOutOfBoundException is caught by MapUtils.java");
	  }
	  if (t1_i == t2_i) return  0;
	  if (t1_i <  t2_i) return -1;
	  if (t1_i >  t2_i) return  1;
  ...
  public static boolean Equal(Map t1, Map t2, AttrType types[], int len)
    throws IOException,UnknowAttrType,BUtilsException
    {
      int i;
  ...
  public static short[] setup_op_map(Map Jmap, AttrType[] res_attrs,
	...
	throw new BUtilsException(e,"setHdr() failed");
      }
